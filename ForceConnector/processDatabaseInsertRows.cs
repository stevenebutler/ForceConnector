...
{
    public partial class processDatabaseInsertRows
    {
        public processDatabaseInsertRows()
        {
            InitializeComponent();
        ...
        private void processDatabaseInsertRows_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void bgw_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                row_counter = 0L;
                string msg = "You try to insert " + totals.ToString("N0") + " records. Are you sure?";
                var result = TopMostMessageBox.Show("Insert Information", msg, MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                ...
                {
                    chunk = excelApp.Intersect((Excel.Range)excelApp.Selection, (Excel.Range)excelApp.ActiveSheet.Rows(row_pointer)); // first row
                    if (chunk is null)
                        break;          // done here
                ...
                if (someFailed)
                {
                    Util.ErrorBox("One or more of the selected rows could not be inserted" + '\n' + "see the comments in the colored cells for details");
                }
        ...
        errors:
            ...
            ;
            excelApp.StatusBar = "Insert : insert complete, " + (row_counter - 1L) + " total rows inserted";
